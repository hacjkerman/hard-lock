...
public struct LockRules {
    ...
    // MARK: - Logical day and cutoff resolution
    ...
    /// Amendment A: this -- not a history of callbacks -- is what decides
    /// whether the phone should be shielded at any given instant.
...
public extension LockRules {
    ...
    /// nil means "never committed"; 0 means "committed, but it has expired".
...
// MARK: - Tighten now, weaken later (Task 3)

public struct ApplyResult: Equatable {
    public let applied: [String]
    public let deferred: [String]
    public let rejected: [String]
    /// Amendment G: keys this config cannot write. Previously these were
    /// reported as `applied` while silently doing nothing.
    public let unknown: [String]
    public let config: LockConfig

    public init(applied: [String], deferred: [String], rejected: [String],
                unknown: [String] = [], config: LockConfig) {
...
public extension LockRules {
    ...
    func cutoffWeakens(oldValue: String?, newValue: String?) -> Bool {
        guard let newValue else { return oldValue != nil }   // removing a limit
    ...
    func apply(changes: [String: String?], now: Date) -> ApplyResult {
        var updated = config
        var applied: [String] = [], deferred: [String] = []
        var rejected: [String] = [], unknown: [String] = []
        ...
        for key in changes.keys.sorted() {
            guard LockConfig.isKnownKey(key) else { unknown.append(key); continue }
        ...
        return ApplyResult(applied: applied, deferred: deferred,
                           rejected: rejected, unknown: unknown, config: updated)
    }
...
public extension LockRules {
    ...
    /// True when the logical day containing `now` uses this cutoff time.
    ///
    /// Amendment A: this is no longer what gates shielding -- `isLockedOut`
    /// is, because it stays correct even when the interval that should have
    /// started never did. Kept for schedule bookkeeping.
